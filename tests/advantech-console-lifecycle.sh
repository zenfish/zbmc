...
grep -Fxq 'ZBMC_REQUIRED_SERVICES="ssh ipmi redfish webui console"' "$repo/boxes/advantech-asmb787/zbmc.box"
...
grep -Fq 'six-service cold boot verified' "$repo/boxes/advantech-asmb787/zbmc.box"
grep -Fq 'Linux SSH verified' "$repo/boxes/advantech-asmb787/zbmc.box"
grep -Fxq 'ZBMC_SSH_PROBE_TIMEOUT=30' "$repo/boxes/advantech-asmb787/zbmc.box"
! grep -qi 'temporarily broken' "$repo/boxes/advantech-asmb787/zbmc.box"
! grep -q 'ZBMC_DEFAULT_NO_WEB=1' "$repo/boxes/advantech-asmb787/zbmc.box"
! grep -q 'NC-SI responder' "$repo/boxes/advantech-asmb787/zbmc.box"
grep -Fq 'zbmc_running() { pgrep -f "ast2600-evb.*ifname=$ZBMC_TAP" | head -1; }' "$repo/boxes/advantech-asmb787/zbmc.box"
grep -Fq 'IPMI_PW=superuser' "$repo/boxes/advantech-asmb787/zbmc.box"
grep -Fq 'user set password 2 superuser --yes' "$repo/boxes/advantech-asmb787/zbmc.box"
grep -Fxq 'COMMIT=45191509417a14e1f823b188cdb3721e7e4ad13f' "$repo/tools/install-zipmi"
grep -Fq 'AUTH OK (Advantech ASMB-787 RMCP+)' "$repo/boxes/advantech-asmb787/zbmc.box"
grep -Fq '"https://$ZBMC_IP:$WEB_PORT/redfish/v1/Systems"' "$repo/boxes/advantech-asmb787/zbmc.box"
grep -Fq '/api/settings/dashboarduser?user_id=$uid&channel_id=1&user_name=$IPMI_USER' "$repo/boxes/advantech-asmb787/zbmc.box"
grep -Fq "printf zbmc-asmb787-linux-shell" "$repo/boxes/advantech-asmb787/zbmc.box"
grep -Fq 'sed -i "/^sysadmin:/s#/usr/local/bin/defshell#/bin/sh#"' "$repo/boxes/advantech-asmb787/qemu-patch-rootfs.sh"
