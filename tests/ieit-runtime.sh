...
grep -Fxq 'ZBMC_TAP=ztap-ieit' "$box/zbmc.box"
grep -Fxq 'ZBMC_MAC=52:54:00:fa:00:41' "$box/zbmc.box"
grep -Fxq 'ZBMC_TIMING_PROFILE=management-with-ssh' "$box/zbmc.box"
grep -Fxq 'SSH_USER=sysadmin' "$box/zbmc.box"
grep -Fq "zbmc_ssh 'printf zbmc-ieit-linux-shell'" "$box/zbmc.box"
...
printf 'sysadmin:x:0:0:sysadmin:/root:/usr/local/bin/defshell\n' >"$conf/passwd"
printf 'sysadmin:old-hash:17823:0:99999:7:::\n' >"$conf/shadow"
printf 'UsePAM yes\nDenyUsers sysadmin\n' >"$conf/ssh_server_config"
printf 'index\n' >"$web/index.html"
printf 'javascript\n' >"$web/source.min.js"
printf 'css\n' >"$web/styles.min.css"
...
grep -Fq 'tap,id=bmcnet,ifname=$TAP,script=no,downscript=no' "$box/boot.sh"
