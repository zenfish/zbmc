...
snapshot="$(dirname "$0")/../boxes/idrac10/boot-live-ckpt.sh"
restore="$(dirname "$0")/../boxes/idrac10/restore-idrac10.sh"
sh -n "$script"
sh -n "$apache_boot" "$apache_setup" "$snapshot" "$restore"
grep -q 'PasswordAuthentication no' "$script"
grep -q 'operator.pub' "$script"
grep -q -- '--prefix=/run/dm --prefix=/flash/data0/config' "$script"
grep -q 'cp -an /flash/pd0/ipmi/evb/.' "$script"
...
grep -q 'bash "$PROJ_DIR/restore-idrac10.sh"' "$box"
grep -q 'ready (cold boot; use start --warm for TAP snapshot)' "$box"
grep -q 'qemu-usb-net.dtb' "$snapshot" "$restore"
grep -q 'runtime-overlay.qcow2' "$box" "$snapshot"
grep -q -- '-qmp "unix:$QMP,server=on,wait=off"' "$box" "$restore"
grep -q 'PYTHONPATH="$ZIPMI" python3 -m zipmi.cli.zipmi' "$restore"
grep -q 'shutil.copy2(overlay, frozen_tmp)' "$snapshot"
grep -q '"device_del".*"tcpnic"' "$snapshot"
! grep -q -- '-device usb-net' "$restore"
grep -q -- '-netdev user,id=tcpnet,net=10.0.3.0/24' "$restore"
grep -A3 '^zbmc_snapshot()' "$box" | grep -q 'zbmc_ipmi_health'
tail -n 5 "$restore" | grep -q 'exit 1'
! grep -q '/phd/tmp/idrac10-virtual' "$snapshot" "$restore"
...
grep -q 'http://10.0.3.2:8091/boot-apache-guest.sh' "$box"
...
grep -q '^ZBMC_NETWORK_MODE=tap$' "$box"
grep -q '^ZBMC_TAP=ztap-idrac10$' "$box"
grep -q '^ZBMC_MAC=52:54:00:fa:00:31$' "$box"
grep -q 'tap,model=npcm-gmac,mac=\$ZBMC_MAC,ifname=\$ZBMC_TAP,script=no,downscript=no' "$box"
! grep -q 'hostfwd=.*\$ZBMC_IP' "$box" "$restore"
grep -q 'for {set i 0} {\$i < 180' "$driver"
...
grep -q 'ip addr add \$guest_ip/\$guest_prefix dev eth0' "$driver"
grep -q 'ip link set eth0 address \$guest_mac' "$driver"
grep -q 'http://10.0.3.2:8091/boot-fullfw-guest.sh' "$driver"
grep -q 'ckpt/network-mode' "$box"
grep -q 'snapshot network topology mismatch' "$restore"
