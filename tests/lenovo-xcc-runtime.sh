...
grep -Fxq 'ZBMC_REQUIRED_SERVICES="ssh ipmi redfish webui console"' "$box/zbmc.box"
! grep -q '^ZBMC_DISABLED_SERVICES=' "$box/zbmc.box"
...
grep -Fq 'python3 "$ROOT/tools/zbmc-configure-serial-ip.py" "$SOCK" "$ZBMC_IP"' "$box/zbmc.box"
