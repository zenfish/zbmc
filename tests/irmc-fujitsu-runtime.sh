...
grep -Fxq 'ZBMC_SSH_PROBE_TIMEOUT=120' "$box/zbmc.box"
...
grep -Fq '[ "$#" -eq 0 ] && exec /bin/sh -i' "$box/build.sh"
grep -Fq 'exec /bin/sh "$@"' "$box/build.sh"
grep -Fq 'conf=/conf/ncml.conf' "$box/build.sh"
grep -Fq 'passwd=/conf/passwd' "$box/build.sh"
grep -Fq 'section && /^current_state=/' "$box/build.sh"
grep -Fq 'print "current_state=1"' "$box/build.sh"
grep -Fq "sed 's/^sysadmin:[^:]*:/sysadmin:x:/'" "$box/build.sh"
grep -Fq "grep -q '^sysadmin:x:'" "$box/build.sh"
grep -Fq 'irmc_ssh_enable || exit 1' "$box/build.sh"
grep -Fq '/newroot/etc/init.d/ssh' "$box/build.sh"
...
grep -Fq 'PubkeyAuthentication=no' "$box/zbmc.box"
grep -Fq 'PreferredAuthentications=password' "$box/zbmc.box"
grep -Fq 'zbmc_ssh_health()' "$box/zbmc.box"
grep -Fq "printf zbmc-irmc-linux-shell" "$box/zbmc.box"
...
grep -Fq 'diagnostic runtime provides authenticated Linux SSH, RMCP+ IPMI' "$box/index.html"
