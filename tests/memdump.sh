...
grep -Fq 'QMP_SOCK="$ZBMC_DIR/qmp.sock"' "$repo/boxes/nvidia-obmc/zbmc.box"
grep -Fq 'qmp = os.path.join(wd, "qmp.sock")' "$repo/boxes/openbmc/start-openbmc.py"
grep -Fq '"-qmp", f"unix:{qmp},server=on,wait=off"' "$repo/boxes/openbmc/start-openbmc.py"
