...
if output=$(./tools/sync-docs 2>&1); then
  echo "default check accepted an unsynchronized pair" >&2
  exit 1
fi
grep -Eq '0/1 were fine, 1 has changed and not synced:' <<<"$output"
grep -Fxq 'test.md' <<<"$output"
grep -Fq 'Type "./tools/sync-docs --write" to sync the docs.' <<<"$output"

output=$(./tools/sync-docs --write 2>&1)
grep -Fq 'wrote test.html' <<<"$output"
grep -Fq '1/1 were fine; all documentation pairs are synced.' <<<"$output"
