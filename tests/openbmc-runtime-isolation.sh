...
check_box(){
  ...
  [[ " $required " == *' webui '* ]]
  if [ -n "$machine" ]; then
    [ "$qemu" = "$repo/qemu/runtime/qemu-system-arm" ]
    [ "$major" = 11 ]
...
grep -Fq '["-,raw,echo=0", f"UNIX-CONNECT:{sock}"]' "$repo/boxes/openbmc/start-openbmc.py"
...
grep -Fxq 'ZBMC_NETWORK_MODE=tap' "$repo/boxes/nvidia-obmc/zbmc.box"
grep -Fxq 'ZBMC_TAP=ztap-nvidia' "$repo/boxes/nvidia-obmc/zbmc.box"
grep -Fxq 'ZBMC_MAC=3c:6d:66:14:c8:7a' "$repo/boxes/nvidia-obmc/zbmc.box"
grep -Fq 'OPENBMC_MACHINE="$ZBMC_QEMU_MACHINE"' "$repo/boxes/nvidia-obmc/zbmc.box"
