...
# -incoming. The VM starts PAUSED; run `ckpt.py restore-finish /tmp/vbmc-idrac9-qmp.sock` to cont
...
exec "${QEMU_BIN:-qemu-system-arm}" -M npcm750-evb -m 1G -display none \
  ...
  -serial mon:stdio \
  -serial unix:/tmp/vbmc-idrac9-ttyS1.sock,server,nowait \
