...
sudo -n "$QEMU" \
  ...
  -chardev "socket,id=serial0,path=/tmp/x14.sock,server=on,wait=off,logfile=$CONSOLE_LOG,logappend=off" \
  -serial chardev:serial0 -qmp unix:/tmp/x14-qmp.sock,server,nowait \
