#!/usr/bin/env bash
# boot-megarac-hpe.sh — boot the HPE Cray XD670 MegaRAC SP-X BMC (AST2600) to a raw root shell (init=/bin/sh).
#
...
#        MegaRAC restricted 'defshell').  For the full stack use boot-megarac-hpe-svc.sh.
...
# RUN  : ./boot-megarac-hpe.sh              (interactive; Ctrl-A x to quit qemu)
