#!/usr/bin/env bash
# snapshot-cray.sh — QMP-checkpoint a GREEN running Cray XD670 BMC to a gz (warm-restore base).
#
...
#      restore-cray.sh then brings that exact green state back in ~10s every time — no reroll.
...
# USAGE: WD=/Users/zen/phd/tmp/cray-xd670 ./snapshot-cray.sh   (qemu must be running with -qmp)
# PAIR:  restore-cray.sh loads it. Snapshot files stay in WD (large; not git — regen via a green boot).
set -euo pipefail
...
echo "snapshot flash -> $WD/cray-snap-flash.bin  (VM is paused; kill it, then restore-cray.sh)"
