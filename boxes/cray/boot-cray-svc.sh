#!/usr/bin/env bash
# boot-cray-svc.sh — boot the HPE Cray XD670 MegaRAC SP-X BMC with REAL init (/sbin/init) + networking.
#
...
# RUN  : ./boot-cray-svc.sh              (foreground, console = your terminal)
#        BG=1 ./boot-cray-svc.sh         (background via a fifo; console -> $WD/svc.log, drive via $WD/cin)
# RELATED: extract.sh, boot-cray.sh, vbmc.box.
set -u
...
# -qmp socket: lets snapshot-cray.sh checkpoint a green instance (QMP migrate->gz). Removed+recreated.
...
if [ "${BG:-}" = 1 ]; then
  ...
  echo "backgrounded. console -> $WD/svc.log ; send input: echo 'cmd' > $WD/cin ; login sysadmin/superuser"
else
  exec "${QEMU[@]}"
fi
