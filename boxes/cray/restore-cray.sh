#!/usr/bin/env bash
# restore-cray.sh — restore a GREEN Cray XD670 BMC from a snapshot in ~10s (IPMI + Redfish live).
#
# WHAT: boots qemu -incoming from cray-snap.gz (made by snapshot-cray.sh), re-attaching the matching
...
# USAGE: IP=10.0.8.67 WD=/Users/zen/phd/tmp/cray-xd670 ./restore-cray.sh   (prints qemu pid)
#        Loopback dev: IP=127.0.0.1 HTTPS_PORT=8443 IPMI_PORT=8623 SSH_PORT=8022 ./restore-cray.sh
set -u
...
[ -f "$SNAP" ] && [ -f "$SNAPFLASH" ] || { echo "no snapshot ($SNAP / $SNAPFLASH) — run snapshot-cray.sh on a green boot first" >&2; exit 1; }
...
$SUDO pkill -9 -f 'qemu-system-arm -M ast2600-evb' 2>/dev/null || true; sleep 2
...
$SUDO qemu-system-arm -M ast2600-evb -m 1024 -display none -no-reboot \
  ...
  -net nic -net "user,hostfwd=tcp:$IP:$HTTPS_PORT-:443,hostfwd=tcp:$IP:$SSH_PORT-:22,hostfwd=udp:$IP:$IPMI_PORT-:623,hostname=craybmc" \
