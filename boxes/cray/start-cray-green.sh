#!/usr/bin/env bash
# start-cray-green.sh — boot the Cray XD670 BMC and RETRY until IPMIMain comes up healthy.
#
...
# RUN: IP=10.0.8.67 WD=/Users/zen/phd/tmp/cray-xd670 ./start-cray-green.sh   (prints qemu pid on green)
...
kill_qemu(){ $SUDO pkill -f 'qemu-system-arm -M ast2600-evb' 2>/dev/null; pkill -f "tail -f $WD/cin" 2>/dev/null; sleep 2; }

for t in $(seq 1 "$TRIES"); do
  echo "[green] boot attempt $t/$TRIES" >&2
  kill_qemu
  HTTPS_PORT="$HTTPS_PORT" SSH_PORT="$SSH_PORT" IPMI_PORT="$IPMI_PORT" BG=1 IP="$IP" WD="$WD" \
    bash "$PROJ/boot-cray-svc.sh" >/dev/null 2>&1
