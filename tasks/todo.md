...
- [x] Select Ubuntu's Python after installing its Crypto package.
...
The original GitHub run failed because the new iLO5 contract requires `Crypto.Cipher.AES`, while the clean Ubuntu runner installed only the cross-compiler. The follow-up run proved that `python3-pycryptodome` installed successfully but the runner still selected a `/usr/local` Python that cannot see Ubuntu's module. The workflow now prepends `/usr/bin` after installing the package so subsequent steps use the matching system Python.
