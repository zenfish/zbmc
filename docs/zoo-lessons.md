...
> current tree, iDRAC9 is cold-only; iDRAC10 cold-boots by default and supports a downloaded, versioned
...
| **idrac10** | Dell iDRAC10 (Yocto + Dell mesh) | Nuvoton **NPCM845** (aarch64); `qemu-system-aarch64 -M npcm845-evb` | `6.12.40` AArch64 (4 binary patches) | direct-kernel `-kernel Image.boot-patched -dtb qemu-usb-net.dtb`; squashfs on sd-bus; explicit downloaded **warm checkpoint** (33s measured) | cold TCP uses `usb-net`; checkpoint removes that non-migratable device and warm host forwards use **npcm-gmac** at 10.0.2.15 | migrated 16550 serial socket | ssh root (matched lab key); IPMI factory IPMIKey `-K`; web root:calvin | ssh, RAKP/IPMI, Redfish | cold READY measured at 257s; build fetches matched RAM/qcow2 state, `snapshot` can replace it, and `start --warm` restores it |
