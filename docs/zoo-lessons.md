...
> current tree, iDRAC9 is cold-only; iDRAC10 cold-boots by default and supports a locally generated
> warm checkpoint; X14 accepts Redfish; and MegaRAC-HPE accepts retained IPMI only. Older results below
...
| **idrac10** | Dell iDRAC10 (Yocto + Dell mesh) | Nuvoton **NPCM845** (aarch64); `qemu-system-aarch64 -M npcm845-evb` | `6.12.40` AArch64 (4 binary patches) | direct-kernel `-kernel Image.boot-patched -dtb qemu-usb-net.dtb`; squashfs on sd-bus; explicit local **warm checkpoint** (33s measured) | cold TCP uses `usb-net`; checkpoint removes that non-migratable device and warm host forwards use **npcm-gmac** at 10.0.2.15 | migrated 16550 serial socket | ssh root (per-installation key); IPMI factory IPMIKey `-K`; web root:calvin | ssh, RAKP/IPMI, Redfish | cold READY measured at 257s; `snapshot` saves RAM plus a frozen **qcow2 overlay**, and `start --warm` restores the matched local pair |
...
| **ilo5** | HPE iLO5 v2.41 (**Green Hills INTEGRITY** RTOS, not Linux) | HPE **GXP** ASIC (Cortex-A9); **Renode** (not QEMU) | INTEGRITY 11.2.4 @0x41000000 | Renode `.repl` model; bl1→kernel; **guarded gate-ledger binary patches** | n/a — no net yet | UART via modeled `gxp_sysctl@0xC0000000` | n/a (pre-app-load) | bootloader→kernel→INTEGRITY scheduler verified; application not loaded | v2.41's bootloader/kernel are byte-identical to v2.33; `elf_secure` is decrypted but not started |
...
- **HPE iLO 5** (`tools/unpack-ilo5`): pins Airbus's GPLv2 toolbox commit and authenticates both AES-GCM envelopes. v2.41 fully decrypts; v2.71 lacks the published `elf_secure` key; v3.19 also changed the outer key. A 2,082-byte output is only the signed header after failed authentication, not clear firmware, and the wrapper rejects it.
...
(cold by default; supported iDRAC10, X14, and MegaRAC snapshots require `start --warm`), `zbmc_build`, `zbmc_snapshot`,
